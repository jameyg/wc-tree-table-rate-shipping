...
class DestinationCondition extends \TrsVendors\Dgm\Shengine\Conditions\Common\AbstractCondition
{
    ...
    private static function extractPostalCodeConstraints($constraint, $country, $state)
    {
        ...
        if (count($parts) == 2 && self::isCountryOrStateListed($country, $state, array(reset($parts)))) {
            return \TrsVendors\Dgm\Arrays\Arrays::map(explode(self::POSTAL_CODE_DELIMITER, end($parts)), 'trim');
        }
